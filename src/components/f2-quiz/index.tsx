/**
 * F2 — Khởi động: trò chơi nhập vai "Physics Quest — Electric City".
 *
...
export const QuizGame: React.FC<QuizGameProps> = ({ userRole, onFinishQuiz, extraQuestions }) => {
  ...
  const bank = useMemo(() => {
    const base = topic ? QUIZ_BANK.filter((q) => q.topic === topic) : QUIZ_BANK;
  ...
  const counts = useMemo(() => {
    ...
    QUIZ_TOPICS.forEach((t) => { map[t.id] = QUIZ_BANK.filter((q) => q.topic === t.id).length; });
    return map;
  }, []);
    ...
    <div className="h-full flex flex-col gap-3 min-h-0">
      ...
      {userRole === 'gv' && (
        <p className="shrink-0 text-[12.5px] text-slate-400 flex items-center gap-1.5">
          ...
          Trò chơi bốc ngẫu nhiên câu hỏi từ ngân hàng {QUIZ_BANK.length} câu của ModuLab, kèm câu hỏi giáo viên bổ sung.
        </p>
      )}
    </div>
  );
