  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-6 gap-4 h-full overflow-y-auto pr-1 pb-4 font-sans select-none">
      ...
      <BentoCard 
        ...
        action={
          <button
            ...
            className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl text-body flex items-center gap-1.5 transition-colors shadow-sm"
          >
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
          <div className="p-3.5 bg-indigo-50 border border-indigo-200 rounded-xl">
            <div className="flex items-center justify-between text-indigo-900 font-semibold text-body mb-1">
              <span>Game ôn tập</span>
              <Award className="w-4 h-4 text-indigo-600" />
            </div>
            <div className="text-h1 font-semibold text-indigo-950">
              {stats.quizCompleted}/{stats.totalStudents} <span className="text-body font-medium text-indigo-700">HS</span>
            </div>
          ...
          <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl">
            <div className="flex items-center justify-between text-emerald-900 font-semibold text-body mb-1">
              <span>Báo cáo nhóm đã nộp</span>
              <CheckCircle2 className="w-4 h-4 text-emerald-600" />
            </div>
            <div className="text-h1 font-semibold text-emerald-950">
              {stats.reportsSubmitted}/10 <span className="text-body font-medium text-emerald-700">Nhóm</span>
            </div>
          ...
          <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl">
            <div className="flex items-center justify-between text-slate-700 font-semibold text-body mb-1">
              <span>Sĩ số & Bảo mật</span>
              <Users className="w-4 h-4 text-slate-500" />
            </div>
            <div className="text-h1 font-semibold text-slate-900">
              {stats.totalStudents} <span className="text-body font-medium text-slate-500">Học sinh</span>
            </div>
            <p className="text-meta text-emerald-600 font-semibold mt-2 flex items-center gap-1">
      ...
      <BentoCard className="md:col-span-2" accent={true} title="QUẢN LÝ GIAO BÀI TẬP">
        <form onSubmit={handleAssign} className="flex flex-col h-full justify-between space-y-3 mt-1 text-body">
          <div>
            <label className="text-meta font-semibold text-indigo-200 block mb-1">Loại bài giao cho lớp:</label>
            <select
              value={assignmentType}
              onChange={(e) => setAssignmentType(e.target.value as any)}
              className="w-full p-2 bg-indigo-900/80 border border-indigo-500 rounded-xl text-white font-semibold outline-none focus:ring-2 focus:ring-white"
            >
            ...
            <label className="text-meta font-semibold text-indigo-200 block mt-3 mb-1">Hạn nộp chính thức:</label>
            <input
              type="date"
              value={dueDate}
          ...
          <button
            type="submit"
            className="w-full py-2.5 bg-white text-indigo-900 font-semibold rounded-xl text-body hover:bg-indigo-50 transition-all flex items-center justify-center gap-1.5 shadow-sm"
          >
      ...
      <BentoCard className="md:col-span-3" title="Thống kê câu hỏi lỗi nhiều nhất" subtitle="Phân tích để giảng dạy lại">
        <div className="mt-2 space-y-3">
          {stats.errorStats.map((err, idx) => (
            <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between gap-3 text-body">
              <div className="flex items-start gap-2 min-w-0">
                <span className="w-5 h-5 rounded bg-rose-100 text-rose-700 flex items-center justify-center font-semibold text-meta shrink-0 mt-0.5">
                  {idx + 1}
                </span>
      ...
      <BentoCard className="md:col-span-3" title="Danh sách học sinh & Tình trạng" subtitle="Theo dõi điểm số lớp">
        <div className="mt-2 overflow-x-auto rounded-xl border border-slate-200 max-h-[260px]">
          <table className="w-full text-left text-body">
            <thead className="bg-slate-100 text-slate-600 font-semibold text-meta sticky top-0 border-b border-slate-200">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {stats.studentList.map((stu) => (
                <tr key={stu.id} className="hover:bg-slate-50">
                  <td className="p-2.5 font-semibold text-slate-800">{stu.name}</td>
                  <td className="p-2.5 text-slate-500 text-meta">{stu.team}</td>
                  ...
                  <td className="p-2.5 text-right">
                    <span className={`px-2 py-0.5 rounded text-meta font-semibold ${ stu.reportStatus === 'Đạt' ? 'bg-emerald-100 text-emerald-800' : stu.reportStatus === 'Chưa đạt' ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-500' }`}>
                      {stu.reportStatus}
                    </span>
                  </td>
                </tr>
