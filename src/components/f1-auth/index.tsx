...
export const AuthAccountView: React.FC<AuthAccountViewProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-6 gap-4 h-full overflow-y-auto pr-1 pb-4 font-sans select-none">
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3 text-body">
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/80 space-y-2.5">
            ...
            <div className="flex justify-between py-1 border-b border-slate-200/60">
              ...
              <span className="font-semibold text-slate-800">{currentUser.teamCode || 'Không áp dụng'}</span>
            </div>
            <div className="flex justify-between py-1">
              <span className="text-slate-400">Chế độ đồng bộ:</span>
              <span className="text-emerald-600 font-semibold">✓ Supabase Realtime</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 pt-4 border-t border-slate-200">
          ...
          <form onSubmit={handleJoinClass} className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-body">
            <div>
              <label className="font-semibold text-slate-600 block mb-1">Mã lớp (6 ký tự):</label>
              <input
                type="text"
                required
                maxLength={8}
                ...
                className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-mono font-semibold outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
            <div>
              <label className="font-semibold text-slate-600 block mb-1">Tên / Mã nhóm (1-6 HS):</label>
              <input
                type="text"
                required
                value={teamInput}
                ...
                className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-semibold outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
      ...
      <BentoCard className="md:col-span-2 flex flex-col justify-between" accent={true} title="Mô phỏng Vai trò CSDL">
          ...
          <div className="space-y-2 mt-4">
            {availableUsers.map((u) => (
              <button
                ...
                className={`w-full p-3 rounded-xl text-left font-semibold transition-all flex items-center justify-between ${ u.id === currentUser.id ? 'bg-white text-slate-900 shadow-lg scale-[1.02]' : 'bg-indigo-800/80 text-indigo-200 hover:bg-indigo-700 border border-indigo-700' }`}
              >
                <div>
                  <div className="text-body">{u.name}</div>
                  <div className="text-meta opacity-75">{u.role === 'gv' ? 'Giáo viên bộ môn' : `HS • ${u.teamCode}`}</div>
                </div>
        ...
        <div className="mt-4 pt-3 border-t border-indigo-500/50 text-meta text-indigo-200 font-medium">
